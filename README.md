...
5. **Use it as a reference and 

## PROGRESSION 1:

--Joins --

1. Write a query to return all rows from user_new table and role tables where there is matching user_new.role_id value in both the usernew and role tables. Display role.name and user_new.name in ascending order
...
5. Write a query to return all user_new.name,skill.name from user_new table where there are matching ids in the tables in the order and constraints given below
-- role
-- profile
-- profile_skills
